    ...
    public AnalyticsSummaryResponse getSummary(Long jobId, int days) {
        ...
        List<JobStat> jobStats = testResultRepository
                ...
                .map(p -> {
                    ...
                    long yest    = p.getYesterdayTotal() != null ? p.getYesterdayTotal() : 0L;
                    long today   = p.getTodayTotal()     != null ? p.getTodayTotal()     : 0L;
                    ...
                    return new JobStat(p.getJobName(), total, p.getLatestBuildStatus(),
                            yest, today, trendLabel, passRate, failRate, noResult);
                })
                .toList();
